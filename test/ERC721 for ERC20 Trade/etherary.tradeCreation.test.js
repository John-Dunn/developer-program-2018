...
contract('Etherary - Creating Trades (ERC721 for ERC20)', function(accounts) {
    ...
    describe("Hook setup", function () {
        it("should have minted the specified amount of token", async function () {
            let owner = await tokenA.ownerOf(tokenAAliceSells);
            let balance = await tokenB.balanceOf(bob);

            assert.equal(owner, alice, "Alice should own the minted token from faucet A");
            assert.equal(balance.toNumber(), amountMinted, "Bob should own the minted token from faucet B");
        });

        it("should have approved the main contract", async function () {
            let approved = await tokenA.getApproved(tokenAAliceSells);
            let allowanceB = await tokenB.allowance(bob, etherary.address);
            assert.equal(approved, etherary.address, "this contract should have been approved for token 0");
            assert.equal(allowanceB.toNumber(), tokenBBobSells, "this contract should have an allowance of A token");
        });
    });
    ...
    describe("Opening a trade", function () {
        ...
        it("should not allow creation of a sell trade with wrong approved token", async function () {
        ...
        it("should allow a trade to be queried", async function () {
            ...
            //     bool isActive;
            // }
            assert.equal(Trade[0], isMakerERC20, "maker contract is ERC721");
            assert.equal(Trade[1], isTakerERC20, "taker contract is ERC721");
